...
- 사용자가 저장된 프롬프트를 찾거나 참고하라고 하면 `prompt-library/catalog.yaml`과
  해당 컬렉션의 README를 먼저 읽고, 목적과 가까운 자료를 최대 세 개만 선택한다.
- 선택한 프롬프트를 그대로 이어 붙이지 말고, 현재 작업에 필요한 구조·제약·검증
  기준만 추출해 새 프롬프트나 내부 실행 명세로 재구성한다.
- 프롬프트를 참고했다면 최종 응답에 사용한 prompt ID를 짧게 밝힌다.
- 사용자가 외부 Skill, MCP, 플러그인 또는 설치 가이드를 찾으면
  `resource-library/catalog.yaml`을 먼저 읽는다. 외부 리소스를 이 저장소의 자체
  Skill처럼 취급하지 않는다.
- 사용자가 발표자료, 웹, 연구 시각화의 색상 조합이나 시각적 분위기를 정해야 하면
  `resource-library/design/color-palettes.md`를 확인한다. 산출물의 성격과 텍스트 대비를
  기준으로 하나를 고르고, 장식용 팔레트를 본문 텍스트 조합으로 오용하지 않는다.
- 외부 리소스 설치 전에는 원본 저장소, 현재 설치 방법, 라이선스와 비밀키 요구사항을
  다시 확인한다. 카탈로그의 과거 명령어를 검증 없이 실행하지 않는다.
...
- 외부 Skill, MCP와 도구 소개 글은 `resource-library/external/`에 두고 원문 전체보다
  출처, 용도, 설치 전 확인사항과 자체 테스트 여부를 기록한다.
